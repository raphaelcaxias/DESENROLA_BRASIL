"""
Desenrola Brasil – Intelligence Platform v2.0 (Final)
...
# ============================================================
# IMPORTS (primeiro, antes de qualquer coisa)
# ============================================================
...
# ============================================================
# CONFIGURAÇÃO DO STREAMLIT (agora st está importado)
# ============================================================
st.set_page_config(
...
# ============================================================
# SESSION STATE INITIALIZATION (após importações)
# ============================================================
...
class DataGenerator:
    ...
    def generate_sample_data(n_records: int = 5000) -> pd.DataFrame:
        ...
        for _ in range(n_records):
            ...
            # CORREÇÃO: usar len(dates) e não len(date)
...
class ChartFactory:
    ...
    def create_seasonal_chart(decomposition: Dict) -> Optional[go.Figure]:
        ...
        fig.update_layout(height=600, title=TEXT["seasonal_decomposition"], showlegend=False)
...
def main():
    ...
    with st.sidebar:
        ...
        with col_t2:
            ...
            new_lang = st.selectbox("Idioma", options=["pt", "en"], index=["pt", "en"].index(LANG),
                                    format_func=lambda x: lang_options[x], key="lang_selector")
            if new_lang != LANG:
        ...
        data_source = st.radio("Fonte", options=["demo", "upload", "local"],
                               format_func=lambda x: {"demo": f"📊 {TEXT['use_demo_data']}",
                                                       "upload": f"📤 {TEXT['upload_csv']}",
                                                       "local": "📁 Arquivo local"}.get(x, x),
                               horizontal=False, label_visibility="collapsed")
        uploaded_file = None
        if data_source == "upload":
            uploaded_file = st.file_uploader("Carregar arquivo", type=["csv", "xlsx", "xls"], label_visibility="collapsed")
        st.markdown("---")
        ...
        sel_tip = st.multiselect("Faixa", sorted(df["tipo_desenrola"].unique()), default=st.session_state.sel_tip, key="sel_tip")
        sel_reg = st.multiselect("Região", sorted(df["regiao"].unique()), default=st.session_state.sel_reg, key="sel_reg")
        sel_seg = st.multiselect("Segmento", sorted(df["tipo_banco"].unique()), default=st.session_state.sel_seg, key="sel_seg")
        ...
        if len(datas) > 1:
            ...
            i0, i1 = st.select_slider("Período", options=list(range(len(datas))),
                                      value=(0, len(datas)-1), format_func=lambda i: date_labels[i])
    ...
    with col1:
        st.metric(label=TEXT["volume"], value=fmt_brl(vol_tot), delta=f"{mom_last:+.1f}% {TEXT['vs_prev_month']}", delta_color="normal")
    with col2:
    ...
    with tab2:
        ...
        with col_m1: st.metric("CR3", f"{cr3:.1f}%", help="Concentração dos 3 maiores bancos")
        with col_m2: st.metric("CR5", f"{cr5:.1f}%", help="Concentração dos 5 maiores bancos")
        with col_m3:
    ...
    with col_exp4:
        json_data = json.dumps({
            ...
            "kpis": {"vol_tot": vol_tot, "ops_tot": ops_tot, "ticket": ticket,
                     "hhi": hhi_val, "cr3": cr3, "cr5": cr5, "gini_regional": gini_r}
        }, indent=2, ensure_ascii=False)
        st.download_button("🔧 JSON (API)", json_data,
