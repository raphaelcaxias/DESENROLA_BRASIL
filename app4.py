"""
Desenrola Brasil – Intelligence Platform v3.0
Versão melhorada com performance otimizada, novos gráficos, exportação PDF e UX refinada.
"""
...
.block-container {{ padding: 1rem 1.5rem !important; max-width: 1600px; margin: 0 auto; }}
.hero {{
...
.hero p {{ font-size: 0.9rem; color: {COLORS["TXT2"]}; margin-bottom: 1rem; }}
.hero-badge {{
...
.insight-text {{ font-size: 0.85rem; color: {COLORS["TXT"]}; line-height: 1.5; }}
.insight-value {{
    font-size: 1.1rem;
    font-weight: 700;
    color: {COLORS["P1"]};
...
.stTabs [data-baseweb="tab"]:hover {{ border-color: {COLORS["P1"]}; color: {COLORS["TXT"]}; }}
.stTabs [aria-selected="true"] {{ background: {COLORS["P1"]}; color: white; border-color: {COLORS["P1"]}; }}
...
def fmt_brl(v: float) -> str:
    ...
    if abs(v) >= 1e9: return f"R$ {v/1e9:.2f}B".replace(".", ",")
    if abs(v) >= 1e6: return f"R$ {v/1e6:.1f}M".replace(".", ",")
    if abs(v) >= 1e3: return f"R$ {v/1e3:.1f}K".replace(".", ",")
...
def classify_bank(name: str) -> str:
    ...
    for keywords, category in rules:
        if any(k in n for k in keywords): return category
    return "Outros"
...
def classify_region(uf: str) -> str:
    ...
    for region, states in mapping.items():
        if uf_upper in states: return region
...
class AnalyticsEngine:
    ...
    def compute_kpis(df: pd.DataFrame) -> Dict[str, Any]:
        ...
        hhi_val = hhi(b_agg)
        cr3 = sum(b_agg.nlargest(3)) / b_agg.sum() * 100 if b_agg.sum() > 0 else 0
        cr5 = sum(b_agg.nlargest(5)) / b_agg.sum() * 100 if b_agg.sum() > 0 else 0
    ...
    def detect_outliers(df: pd.DataFrame, column: str = "volume_operacoes") -> Tuple[pd.DataFrame, int]:
        try:
            ...
            iso_forest = IsolationForest(contamination=CONFIG.outlier_contamination, random_state=42)
            df = df.copy()
            df["is_outlier"] = iso_forest.fit_predict(features) == -1
    ...
    def seasonal_decomposition(evol_g: pd.DataFrame) -> Optional[Dict]:
        try:
            series = evol_g.set_index("data_base")["volume_operacoes"].asfreq("MS")
            if len(series) < 12: return None
            result = seasonal_decompose(series, model="additive", period=12)
            return {"trend": result.trend, "seasonal": result.seasonal, 
                    "resid": result.resid, "observed": result.observed}
        except Exception as e:
    ...
    def forecast_series(evol_g: pd.DataFrame, periods: int = 3) -> Optional[Dict]:
        try:
            ...
            return {"forecast": prev, "dates": dt_fut, "lower": prev - 1.96*sigma, "upper": prev + 1.96*sigma}
        except Exception as e:
            logger.warning(f"Forecast failed: {e}")
            return None

# ============================================================
# CHART FACTORY (CACHED)
# ============================================================
class ChartFactory:
    @staticmethod
    @st.cache_data
    def create_timeline_chart(df: pd.DataFrame, evol_g: pd.DataFrame) -> go.Figure:
        ...
        if forecast:
            ...
            fig.add_trace(go.Scatter(
                ...
                fillcolor="rgba(251,191,36,0.15)", line=dict(color="rgba(0,0,0,0)"),
        ...
        fig.update_layout(title=TEXT["time_series"])
    ...
    def create_heatmap(df: pd.DataFrame) -> Optional[go.Figure]:
        try:
            ...
            if heat_data.empty: return None
            ...
            fig.update_layout(title=TEXT["heatmap_title"])
            return fig
        except Exception:
            return None
    ...
    def create_concentration_chart(df: pd.DataFrame, top_n: int) -> go.Figure:
        ...
        segment_colors = {"Tradicional": COLORS["P1"], "Digital": COLORS["AZUL"],
                         "Investimento": COLORS["ROXO"], "Cooperativa": COLORS["VERDE"],
                         "Fintech": COLORS["AMBER"], "Outros": COLORS["CINZA"]}
        ...
        fig = go.Figure(go.Bar(
            ...
            text=banco_agg["volume"].apply(fmt_brl), textposition="outside",
        ...
        fig.update_layout(title=f"Top {top_n} Instituições")
    ...
    def create_brazil_map(df: pd.DataFrame) -> go.Figure:
        ...
        if geojson:
            featureidkey = "properties.sigla" if "sigla" in geojson["features"][0]["properties"] else "properties.name"
            fig = px.choropleth(uf_data, geojson=geojson, locations="uf",
                                featureidkey=featureidkey, color="volume",
                                color_continuous_scale="Blues",
                                title="Distribuição Regional",
                                hover_name="uf", hover_data={"volume": ":,.0f"})
            fig.update_geos(fitbounds="locations", visible=False)
            fig.update_layout(height=500, margin=dict(l=0, r=0, t=40, b=0),
                              template=COLORS["TPLOTE"], paper_bgcolor="rgba(0,0,0,0)")
            return fig
        else:
            fig = go.Figure()
            fig.add_annotation(text=TEXT["map_unavailable"], x=0.5, y=0.5, showarrow=False,
    ...
    def create_regional_bar(df: pd.DataFrame) -> go.Figure:
        ...
        fig.add_trace(go.Bar(x=reg_data["volume"], y=reg_data["regiao"],
                             name="Volume", orientation="h",
                             marker_color=CHART_COLORS[:len(reg_data)],
                             text=reg_data["volume"].apply(fmt_brl), textposition="outside"), secondary_y=False)
        fig.add_trace(go.Scatter(x=reg_data["ticket"], y=reg_data["regiao"],
                                 ...
                                 xaxis="x2"), secondary_y=True)
        fig.update_layout(title=TEXT["volume_and_ticket_by_region"],
    ...
    def create_regional_donut(df: pd.DataFrame) -> go.Figure:
        ...
        fig = go.Figure(go.Pie(labels=reg_data["regiao"], values=reg_data["volume_operacoes"],
                               hole=0.55,
        ...
        fig.update_layout(title="Participação Regional", height=400)
    ...
    def create_cluster_chart(df: pd.DataFrame) -> Optional[go.Figure]:
        try:
            ...
            if len(cl_df) < CONFIG.min_cluster_samples: return None
            ...
            for c in cl_df["cluster"].unique():
                ...
                fig.add_trace(go.Scatter(
                    ...
                    name=f"Cluster {c+1}",
                    marker=dict(size=sz, color=CHART_COLORS[c % len(CHART_COLORS)],
    ...
    def create_radar_chart(hhi_val: float, cr3: float, cr5: float, gini_r: float, ticket: float) -> go.Figure:
        ...
        fig = px.line_polar(radar_data, r="Valor", theta="Métrica", line_close=True,
                            color_discrete_sequence=[COLORS["P1"]], title="Radar de Concentração")
        fig.update_traces(fill="toself", fillcolor=f"rgba(63,182,140,0.2)")
        fig.update_layout(template=COLORS["TPLOTE"], height=450,
                          polar=dict(radialaxis=dict(visible=True, range=[0, 1], gridcolor=COLORS["GRID"]),
    ...
    def create_scatter_chart(df: pd.DataFrame) -> go.Figure:
        ...
        fig = px.scatter(sc_df, x="ms", y="ticket", size="vol", color="ticket",
                         ...
                         title="Ticket Médio vs Market Share",
    ...
    def create_outlier_chart(df: pd.DataFrame) -> Optional[go.Figure]:
        if "is_outlier" not in df.columns: return None
        fig = go.Figure()
        normal = df[~df["is_outlier"]]
        fig.add_trace(go.Scatter(x=normal["numero_operacoes"], y=normal["volume_operacoes"],
        ...
        if not outliers.empty:
            fig.add_trace(go.Scatter(x=outliers["numero_operacoes"], y=outliers["volume_operacoes"],
        ...
        fig.update_layout(title=TEXT["outlier_detection"],
                          xaxis_title="Número de Operações", yaxis_title="Volume (R$)")
        base_layout(fig, h=450)
        return fig
    ...
    def create_pareto_chart(df: pd.DataFrame) -> Tuple[go.Figure, int]:
        ...
        fig = make_subplots(specs=[[{"secondary_y": True}]])
        fig.add_trace(go.Bar(x=pareto_display["nome_conglomerado_financeiro"].str[:20],
                             y=pareto_display["volume_operacoes"], name="Volume",
                             marker_color=COLORS["P1"],
                             text=pareto_display["volume_operacoes"].apply(fmt_brl),
                             textposition="outside"), secondary_y=False)
        fig.add_trace(go.Scatter(x=pareto_display["nome_conglomerado_financeiro"].str[:20],
                                 y=pareto_display["acum"], name="% Acumulado",
                                 mode="lines+markers",
                                 line=dict(color=COLORS["AMBER"], width=2.5),
                                 marker=dict(size=8, color=COLORS["AMBER"])), secondary_y=True)
        fig.add_hline(y=80, line_dash="dot", line_color=COLORS["VERM"],
                     secondary_y=True, annotation_text="80%")
        fig.update_layout(title="Curva de Pareto")
        base_layout(fig, h=450)
    ...
    def create_seasonal_chart(decomposition: Dict) -> Optional[go.Figure]:
        if decomposition is None: return None
        ...
        fig.add_trace(go.Scatter(x=decomposition["observed"].index, y=decomposition["observed"],
                                 mode="lines", name="Observado", line=dict(color=COLORS["P1"], width=1.5)), row=1, col=1)
        fig.add_trace(go.Scatter(x=decomposition["trend"].index, y=decomposition["trend"],
                                 mode="lines", name="Tendência", line=dict(color=COLORS["AMBER"], width=2)), row=2, col=1)
        fig.add_trace(go.Scatter(x=decomposition["seasonal"].index, y=decomposition["seasonal"],
                                 ...
                                 fill="tozeroy", fillcolor="rgba(59,130,246,0.2)"), row=3, col=1)
        fig.add_trace(go.Scatter(x=decomposition["resid"].index, y=decomposition["resid"],
                                 mode="lines", name="Resíduo", line=dict(color=COLORS["VERM"], width=1)), row=4, col=1)
        fig.update_layout(height=600, title="Decomposição Sazonal", showlegend=False)
...
class ExportManager:
    ...
    @staticmethod
    def create_pdf_report(figures: List[go.Figure]) -> Optional[bytes]:
        """Exporta gráficos como PNG em um único arquivo PDF (simplificado)."""
        try:
            import io
            ...
            for i, fig in enumerate(figures):
                if i > 0:
                    c.showPage()
                # Exporta figura como PNG
                img_bytes = fig.to_image(format="png", width=800, height=400, scale=1.5)
                img_io = io.BytesIO(img_bytes)
                img_reader = ImageReader(img_io)
                c.drawImage(img_reader, 50, 100, width=500, height=350, preserveAspectRatio=True)
                c.drawString(50, 500, f"Gráfico {i+1}")
...
def main():
    ...
    with st.sidebar:
        ...
        <div style="text-align:center; padding:0.5rem 0 1rem; border-bottom:1px solid {COLORS['BORDA']}; margin-bottom:1rem;">
            <div style="font-family:'Inter',sans-serif; font-size:1.3rem; font-weight:700; color:{COLORS['P1']};">🏦 {TEXT['app_title']}</div>
        ...
        with col_t2:
            ...
            new_lang = st.selectbox("Idioma", options=["pt", "en"],
        ...
        st.markdown("---")
        st.markdown(f"### 📂 Fonte de Dados")
        
        data_source = st.radio("Fonte", options=["demo", "upload", "local"],
                               format_func=lambda x: {
                                   "demo": f"📊 Dados de demonstração",
                                   "upload": f"📤 Carregar CSV",
                                   "local": "📁 Arquivo local"
                               }.get(x, x),
        ...
        if data_source == "upload":
            uploaded_file = st.file_uploader("Carregar arquivo", type=["csv", "xlsx", "xls"],
                                             label_visibility="collapsed")
        ...
        sel_tip = st.multiselect("Faixa", sorted(df["tipo_desenrola"].unique()),
                                 default=st.session_state.sel_tip, key="sel_tip")
        sel_reg = st.multiselect("Região", sorted(df["regiao"].unique()),
                                 default=st.session_state.sel_reg, key="sel_reg")
        sel_seg = st.multiselect("Segmento", sorted(df["tipo_banco"].unique()),
                                 default=st.session_state.sel_seg, key="sel_seg")
        ...
        if st.button(f"🔄 Resetar Filtros", use_container_width=True):
    ...
    <div class="hero">
        ...
        <div>
            <span class="hero-badge">📊 {fmt_num(kpis['ops_tot'])} contratos</span>
            <span class="hero-badge">💰 {fmt_brl(kpis['vol_tot'])} renegociados</span>
            <span class="hero-badge">🏛️ {kpis['n_banks']} instituições</span>
            <span class="hero-badge">🗺️ {kpis['n_uf']} estados</span>
    ...
    with col1:
        st.metric(TEXT["volume"], fmt_brl(kpis["vol_tot"]),
                  delta=f"{kpis['mom_last']:+.1f}% {TEXT['vs_prev_month']}", delta_color="normal")
    with col2:
        st.metric(TEXT["contracts"], fmt_num(kpis["ops_tot"]))
    with col3:
        st.metric(TEXT["avg_ticket"], fmt_brl(kpis["ticket"]))
    with col4:
        st.metric(TEXT["institutions"], fmt_num(kpis["n_banks"]))
    with col5:
        st.metric(TEXT["states"], fmt_num(kpis["n_uf"]))
    ...
    cresc_med = kpis["evol_g"]["mom"].dropna().mean() if not kpis["evol_g"]["mom"].dropna().empty else 0.0
    ...
    with insight_cols[1]:
        if area_leader is not None:
            ...
            """, unsafe_allow_html=True)
    with insight_cols[2]:
        corr_val = corr_matrix.loc["volume_operacoes", "ticket_medio"] if not corr_matrix.empty and "volume_operacoes" in corr_matrix.columns and "ticket_medio" in corr_matrix.columns else 0.0
        st.markdown(f"""
        <div class="insight-card">
            <div class="insight-title">🔗 Correlação</div>
            <div class="insight-text">{TEXT['insight_correlation'].format(corr_val)}</div>
            <div class="insight-value">{TEXT['insight_top_banks'].format(kpis['cr3'])}</div>
        </div>
        """, unsafe_allow_html=True)
    ...
    # --- TAB 1 ---
    with tab1:
        st.markdown(f"### 📈 {TEXT['time_series']}")
        col_ev1, col_ev2 = st.columns([2, 1])
        with col_ev1:
            fig1 = ChartFactory.create_timeline_chart(dff, kpis["evol_g"])
            st.plotly_chart(fig1, use_container_width=True)
        with col_ev2:
            fig2 = ChartFactory.create_mom_chart(kpis["evol_g"])
        ...
        if seasonal:
            st.markdown(f"**📊 Decomposição Sazonal**")
    ...
    with tab2:
        ...
        top_n = st.slider("Top N instituições", 5, 30, CONFIG.default_top_n, key="top_n_bancos")
        ...
        with col_m1:
            st.metric("CR3", f"{kpis['cr3']:.1f}%", help="Concentração dos 3 maiores bancos")
        with col_m2:
            st.metric("CR5", f"{kpis['cr5']:.1f}%", help="Concentração dos 5 maiores bancos")
        with col_m3:
    ...
    # --- TAB 3 ---
    with tab3:
        st.markdown(f"### 🗺️ {TEXT['regional_analysis']}")
    ...
    # --- TAB 4 ---
    with tab4:
        st.markdown(f"### 🔬 {TEXT['advanced_analytics']}")
        ...
        with col_a1:
            fig_radar = ChartFactory.create_radar_chart(kpis["hhi"], kpis["cr3"], kpis["cr5"],
                                                        kpis["gini_r"], kpis["ticket"])
    ...
    # --- TAB 5 ---
    with tab5:
        st.markdown(f"### 📊 {TEXT['distribution']}")
    ...
    <div class="footer">
        ...
        <span style="font-size:0.55rem; color:{COLORS['TXT2']};">v3.0 | Built with Streamlit & Plotly</span>
    </div>
    """, unsafe_allow_html=True)
